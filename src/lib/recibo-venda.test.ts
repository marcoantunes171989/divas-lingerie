...
describe('Gerador de Recibo PDF', () => {
  ...
  it('deve gerar o cabeçalho da loja e o tipo de cupom', async () => {
    ...
    expect(mockText).toHaveBeenCalledWith('DIVAS LINGERIE', expect.any(Number), expect.any(Number), { align: 'center' });
    expect(mockText).toHaveBeenCalledWith('*** CUPOM NAO FISCAL ***', expect.any(Number), expect.any(Number), { align: 'center' });
  });

  it('deve incluir o número do cupom fiscal e o cliente', async () => {
    await gerarReciboVendaPDF(mockData);

    expect(mockText).toHaveBeenCalledWith('N 123456', expect.any(Number), expect.any(Number), { align: 'right' });
