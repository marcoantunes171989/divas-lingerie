    ...
    try {
      ...
      if (selected) {
        const { error } = await supabase
          .from("tab_motivos_cancelamento")
          .update({
            mot_descricao: formData.mot_descricao.trim(),
          ...
          <form onSubmit={handleSave}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <DialogDescription className="text-slate-400">
                      {selected
                        ? `Código: ${formatCodigo(selected.mot_codigo)}`
            ...
            <div className="p-6 space-y-4">
              <div className="grid gap-2">
                <Label htmlFor="mot_descricao" className="font-bold">
                  Descrição *
                </Label>
                <Textarea
                  id="mot_descricao"
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent className="rounded-3xl border-none shadow-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Deseja excluir o motivo{" "}
              <span className="font-bold text-slate-900">"{selected?.mot_descricao}"</span>? Esta
              ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-3 mt-6">
            <AlertDialogCancel className="rounded-xl h-12 border-slate-200 text-slate-600">
              Cancelar
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={(e) => {
