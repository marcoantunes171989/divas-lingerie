...
function ProdutosPage() {
  ...
  useEffect(() => {
    if (selectedProduto) {
      setFormData({
        ...
        pro_valor_venda:
          selectedProduto.pro_valor_venda?.toLocaleString("pt-BR", { minimumFractionDigits: 2 }) ||
          "0,00",
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const valorVenda =
        parseFloat(formData.pro_valor_venda.replace(/\./g, "").replace(",", ".")) || 0;
      const estoqueAtual = parseInt(formData.pro_estoque_atual) || 0;

      if (valorVenda <= 0) {
        toast.error("O preço de venda é obrigatório e deve ser maior que zero.");
        setSubmitting(false);
        return;
      }
      if (formData.pro_estoque_atual.trim() === "" || isNaN(estoqueAtual)) {
        toast.error("O estoque atual é obrigatório.");
        setSubmitting(false);
        return;
      }
      if (estoqueAtual < 0) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (salesData && salesData.length > 0) {
        setDependencyError(
          "Este produto possui vendas concluídas ou ativas e não pode ser excluído. Somente produtos sem vendas ou com vendas canceladas podem ser removidos.",
        );
        return;
      }

      const { error } = await supabase.from("tab_produtos").delete().eq("id", selectedProduto.id);
              ...
              {paginatedProdutos.map((prod) => (
                <TableRow key={prod.id} className="hover:bg-slate-50 transition-colors">
                  <TableCell>
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p
                          className="font-bold text-slate-900 truncate text-sm sm:text-base mb-0.5"
                          title={prod.pro_descricao}
                        >
                          {prod.pro_descricao}
                        </p>
                        <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                          ...
                          <span
                            className={`text-[10px] sm:text-xs font-medium sm:hidden ${(prod.pro_estoque_atual || 0) < (prod.pro_estoque_minimo || 0) ? "text-red-500" : "text-slate-500"}`}
                          >
                  ...
                  <TableCell className="hidden sm:table-cell">
                    <Badge
                      variant="secondary"
                      className="rounded-full px-2 py-0 sm:px-3 text-[10px] sm:text-xs truncate max-w-[100px]"
                    >
                      {prod.tab_categorias?.cat_nome}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="text-center hidden sm:table-cell">
                    <div className="flex items-center justify-center gap-1">
                      <span
                        className={`font-bold text-sm sm:text-base ${(prod.pro_estoque_atual || 0) < (prod.pro_estoque_minimo || 0) ? "text-red-500" : "text-slate-900"}`}
                      >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <DialogDescription className="text-slate-400">
                      {selectedProduto
                        ? `Editando: ${selectedProduto.pro_descricao}`
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, pro_codigo_barras: e.target.value })
                        }
                        className="rounded-xl h-12 bg-slate-50 border-slate-200 focus:bg-white transition-all"
                      />
                    </div>
                  ...
                  <div className="grid gap-4">
                    <div className="grid gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          setFormData({ ...formData, pro_categoria_id: value })
                        }
                      >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="grid gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value) =>
                            setFormData({ ...formData, pro_tamanho_id: value })
                          }
                        >
                          <SelectTrigger className="rounded-xl h-12 bg-slate-50 border-slate-200">
                          ...
                          <SelectContent className="rounded-xl border-none shadow-xl">
                            {tamanhos.map((tam) => (
                              <SelectItem key={tam.id} value={tam.id}>
                                {tam.tam_nome}
                              </SelectItem>
                            ))}
                          </SelectContent>
            ...
            <AlertDialogDescription className="text-slate-600 text-base">
              {dependencyError ? (
                ...
                <>
                  ...
                  <span className="font-bold text-slate-900">{selectedProduto?.pro_descricao}</span>
