-- Inclui o valor por produto (valor_unitario e valor_cancelado) no snapshot de cancelamento,
-- para o Relatório de Cancelamentos exibir o valor cancelado de cada produto.
-- (Já aplicada no banco via Supabase MCP; este arquivo documenta/versiona.)
CREATE OR REPLACE FUNCTION public.cancelar_venda(
  p_venda_id uuid, p_motivo text DEFAULT NULL::text, p_cupom_fiscal text DEFAULT NULL::text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    FOR v_item IN
        SELECT iv.id, iv.itv_produto_id, iv.itv_quantidade,
               iv.itv_valor_unitario, iv.itv_valor_total,
    ...
    LOOP
        v_estoque_snapshot := v_estoque_snapshot || jsonb_build_array(
            jsonb_build_object(
                ...
                'valor_unitario',          v_item.itv_valor_unitario,
                'valor_cancelado',         v_item.itv_valor_total
            )
        );
